...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    db.run('INSERT INTO suppliers (name, contact, phone, address) VALUES (?, ?, ?, ?)',
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const orders = db.exec('SELECT id FROM purchase_orders WHERE supplier_id = ? LIMIT 1', [req.params.id]);
